...
- [x] **UI 렌더링 엔진 개발**
    ...
    - [x] **타이포그래피:** 한글 폰트 로드(다중 폰트 지원), 캐싱 및 줄바꿈 처리
    - [ ] **이미지 합성:** 말풍선 + 텍스트 + 아이콘 합성 로직

### Step 4: 통합 및 자동화
*목표: 전원 인가 시 프로그램이 자동으로 동작하도록 만듭니다.*
- [x] **메인 루프(Main Loop) 완성:** [데이터 수신] -> [판단] -> [렌더링] -> [갱신] -> [Sleep]
- [x] **예외 처리:** 네트워크 타임아웃(10s) 및 Watchdog(60s) 적용으로 멈춤 현상 방지
- [x] **자동 실행:** `systemd` 서비스 등록 (`plantalk.service`) 및 Watchdog 설정 추가
